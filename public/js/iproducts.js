...
function LoadCboType() {
	var selectElemRef = document.getElementById("cboType");
	var objProductsType;
	$.ajax({
		dataType: "json",
		url: "/getProductType",
		data: objProductsType,
		success: function (data) {
			objProductsType = data;
			var html = '';
			
			while (selectElemRef.length > 0) {
				selectElemRef.remove(0);
			}
			var oTemp = new Option("Chọn",0);
			$("#cboType").append(oTemp);
			for (var i = 0, len = objProductsType.length; i < len; ++i) {
				var o = new Option(objProductsType[i].Name, objProductsType[i]._id);
				//o.selected=true;
				$("#cboType").append(o);
			}
...
//	};
	...
	objProduct.TypeId = cboType.value;
	objProduct.Type = cboType.options[cboType.selectedIndex].text;
	...
	//// Type Product int 
	form.append('TypeId', objProduct.TypeId);
	//// Name Product string
