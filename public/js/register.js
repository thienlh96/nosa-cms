...
function loadPosition(){
	...
	$.ajax({
		...
		success: function (data) {
			...
			document.getElementById("cboLevel").selectedIndex = "1";
			selectCTHLHTNT();
...
//	};
	...
	var strName= removeChar(txtFullName.value);
	strName=strName.replace(/\s+/g, '').toLowerCase();
	if(cboLevel.value=='1'){
		if(cboPosition.options[cboPosition.selectedIndex].text=='Chủ tịch'){
			if(strName!='lequocphong')
				{
					alert("Bạn chọn chức danh không phù hợp.");
					cboPosition.focus();
					btnSend.disabled = false;
					btnSend.style.color = '#FFFFFF';
					return;
